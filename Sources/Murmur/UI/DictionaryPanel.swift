...
///
/// Below the dictionary rather than above it: these are suggestions, and the entries the user
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: DS.Space.tight) {
            VStack(alignment: .leading, spacing: DS.Space.hair) {
                Silkscreen(text: "Learned from your speech", color: DS.Color.inkOnDeck.opacity(0.5))
                Text("Already priming the engine. Keep one to make it permanent, ignore it to stop being offered it.")
                    .font(DS.Font.label)
                    .foregroundStyle(DS.Color.inkOnDeck.opacity(0.4))
...
private struct LearnedRow: View {
    ...
    /// Amber, not the green the dictionary rows use: this is live but provisional, and the
    /// difference is the whole point of showing it.
    var body: some View {
        HStack(spacing: DS.Space.base) {
            Lamp(color: DS.Color.meterAmber, isLit: true, size: 6)

            Silkscreen(text: "Heard", color: DS.Color.inkOnDeck.opacity(0.5))
                .frame(width: 34, alignment: .leading)
            ...
            Silkscreen(
                ...
                color: DS.Color.inkOnDeck.opacity(0.4)
            )

            if isHovering {
                rowButton("Keep", action: onKeep)
                rowButton("Ignore", action: onIgnore)
            }
        }
        .padding(.horizontal, DS.Space.base)
        .padding(.vertical, DS.Space.snug)
